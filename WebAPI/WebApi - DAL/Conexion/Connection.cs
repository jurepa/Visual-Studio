...
namespace WebApi___DAL.Conexion
{
    public class Connection
    {
        ...
        public Connection()
        {
            ...
            try
            {
                ...
                this.conexion.ConnectionString = string.Format($"server=pokemonjarana.database.windows.net;database={dataBase};uid={user};pwd={pass};");
                this.conexion.Open();
            }
            catch (SqlException)
            {
        ...
        public Connection(String database, String user, String pass)
        {
            ...
            try
            {
                ...
                //connection.ConnectionString = "server=(local);database=" + dataBase + ";uid=" + user + ";pwd=" + pass + ";";
                //Muy cómoda esta forma de escribir la cadena conStringFormat, metiendo los parametros entre llaves y asignandoselo tras la coma
                this.conexion.ConnectionString = string.Format("server=(local);database={0};uid={1};pwd={2};", dataBase, user, pass);
                this.conexion.Open();
            }
            catch (SqlException)
            {
